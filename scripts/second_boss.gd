...
func create_body():
	for line : Line2D in body.get_children():
		line.points = create_random_rect()
		line.position = Vector2(randi_range(-16, 16 - line.points[1].x), randi_range(-16, 16 - line.points[2].y))
		line.default_color = [primary_color, secondary_color, tertiary_color].pick_random()
...
func flip_parts():
	...
	while parts_array.size() > 1:	
		...
		var rect_1 : Line2D = body.get_child(x)
		var rect_2 : Line2D = body.get_child(y)
...
func knockback(dir : Vector2):
	for rect : Line2D in body.get_children():
		var prev_color : Color = rect.default_color
